...
if response.status_code == 200:
    # 逐行查找匹配的内容
    lines = response.text.split('\n')
    is_in_wstv_section = False
    channels = {}
    for line in lines:
        if "卫视,#genre#" in line:
            is_in_wstv_section = True
            continue 
        elif "卫视" in line and is_in_wstv_section:
            parts = line.split(",")
            if len(parts) >= 2:
                channel_name = parts[0].strip()
                channel_url = parts[1].strip()
                channels[channel_name] = channel_url
        elif "卫视" not in line and is_in_wstv_section:
            break  # 央视频道部分结束

    # 检查是否找到了所需的信息
    if channels:
        # 写入到本地文件
        with open("wstv.txt", "w", encoding="utf-8") as f:
            f.write(\n\n卫视频道,#genre#\n")
            for channel, url in channels.items():
                f.write("{},{}\n".format(channel, url))
        print("卫视频道的频道信息已写入到 wstv.txt")
