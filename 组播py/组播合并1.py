...
with open('酒店源.txt', 'r', encoding='utf-8') as f:  #打开文件，并对其进行关键词提取                                               ###########
 ...
 keywords = ['重温', '酒店', '私人', '天映', '莲花', 'AXN', '好莱坞', '星', '龙祥', '凤凰', '有线电', '东森']  # 需要提取的关键字列表                                                       ###########
 pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字                                      ###########
 #pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制                                                     ###########
...
file_contents = []
file_paths = ["a0.txt", "b.txt", "b1.txt", "df0.txt", "f.txt", "f1.txt"]  # 替换为实际的文件路径列表#############################################
for file_path in file_paths:                                                             #############################################
    with open(file_path, 'r', encoding="utf-8") as file:                                 #############################################
        content = file.read()
        file_contents.append(content)
...
#任务结束，删除不必要的过程文件###########################################################################################################################
files_to_remove = ['湖南电信.txt', '广东电信.txt', '四川电信.txt', '河南电信.txt', '河北电信.txt', '天津联通.txt', '组播源.txt', "GAT.txt", "TW.txt", "a.txt", "a0.txt", "b.txt", "b1.txt", "港澳.txt", "df0.txt", "df.txt", "df1.txt", "sr1.txt", "sr2.txt", \
