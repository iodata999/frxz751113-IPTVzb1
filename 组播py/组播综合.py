...
##########################################################################E#####################################################################################
keywords = ['风云', '兵器', '女性', '地理', '央视文化', '风云', '剧', '院线', '影', 'CHC', '星空', '凤凰']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
...
# 读取要合并的频道文件，并生成临时文件##############################################################################################################
file_contents = []
file_paths = ["TT.txt", "b.txt", "a.txt", "c.txt", "c1.txt", "e.txt", "DD.txt", "df.txt", "df1.txt", "f.txt"]  # 替换为实际的文件路径列表
for file_path in file_paths:
    with open(file_path, 'r', encoding="utf-8") as file:
        content = file.read()
        file_contents.append(content)
...
files_to_remove = ['湖南电信.txt', '广东电信.txt', '湖南电信.m3u', '河南电信.m3u', '四川电信.m3u', '广东电信.m3u', '天津联通.m3u', '河北电信.m3u', \
                   "GAT.txt", "DD.txt", "TW.txt", "a.txt", "b.txt", "c.txt", "c1.txt", "e.txt", "f.txt", "df.txt", "df1.txt", "TT.txt", "ott移动v4.txt"]
