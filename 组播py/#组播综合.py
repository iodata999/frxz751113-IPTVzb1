...
keywords = ['凤凰中文', 'TVB星河', '八大', 'AMC,', 'amc电影台', 'AMC电影台', 'amc電影台', 'AMC電影台']  # 需要提取的关键字列表
pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
...
for line in fileinput.input("e.txt", inplace=True):  #打开文件，并对其进行关键词原地替换                     ###########
    ...
    line = line.replace("AMC,", "AMC电影台,")                                                                         ###########
    line = line.replace("amc", "AMC")                                                                         ###########
    line = line.replace("AA", "")                                                                         ###########
    line = line.replace("龙华电影,", "龙华电影台,")                                                                         ###########
    print(line, end="")  #设置end=""，避免输出多余的换行符   
...
# 读取要合并的频道文件，并生成临时文件##############################################################################################################
file_contents = []
file_paths = ["b.txt", "e.txt", "a.txt", "d.txt", "c.txt", "ws.txt", "ys.txt", "DD.txt", "df.txt"]  # 替换为实际的文件路径列表
for file_path in file_paths:
    with open(file_path, 'r', encoding="utf-8") as file:
        content = file.read()
        file_contents.append(content)
